...
public class UrlUtil {
    public static String AddOrModifyQueryByKey(String url, String key, String newValue,String[] whitUrlHost) {
        try {
            URI uri;
            URL webUrl = new URL(url);
            String webHost = webUrl.getHost();
            boolean flag = true;
            for (String whitUrl : whitUrlHost) {
                if (webHost.endsWith(whitUrl)) {
                    flag = false;
                }
            }
            if (flag) {
